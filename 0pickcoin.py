...
ticker1 = str('"'+ticker0+'"')
ticker2 = str('"'+ticker0[0:3]+'"')
ticker3 = str('"'+ticker0[4:]+'"')
print(ticker1)
print(ticker2)
print(ticker3)
...
while True:
    ...
    try:
        ...
        print("BTC보유량 : " + str(upbit.get_balance(ticker1)))     # KRW-XRP 조회
        print("KRW보유량 : " + str(int(upbit.get_balance(ticker2))))         # 보유 현금 조회
        
        start_time = get_start_time(ticker1)
        ...
        now_price = get_current_price(ticker1)
        print("현재가격 : " + str(now_price))

        if start_time < now < start_time - datetime.timedelta(hours=6):
            wish_price = get_target_price(ticker1, 0.6)
        if start_time - datetime.timedelta(hours=6) < now < end_time - datetime.timedelta(seconds=10):
            wish_price = get_target_price2(ticker1, 0.2)
        print("매수목표가격 : " + str(wish_price) )

        get_sell_price(ticker1)
        print("==========================================")
